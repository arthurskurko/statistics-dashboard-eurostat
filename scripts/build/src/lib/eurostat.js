...
const topicCatalog_1 = require("../features/dashboard/topicCatalog");
const EUROSTAT_BASE = 'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data';
function buildUrl(topicId) {
    const topic = topicCatalog_1.TOPIC_MAP[topicId];
    if (!topic) {
...
function unravelIndex(flatIndex, sizes) {
...
function parseSeries(dataset, fallbackLabel) {
    ...
    const values = Array.isArray(dataset.value)
        ? dataset.value.map((value, index) => [index, value])
        : Object.entries(dataset.value).map(([index, value]) => [Number(index), value]);
...
function buildAggregate(dataset, includeCodes, label) {
    ...
    for (const [flatIndex, rawValue] of values) {
        ...
        if (!includeCodes.includes(geoCode))
            continue;
...
async function fetchTopicData(topicId) {
    ...
    if (topic.geoValues?.includes('EU27_2020') &&
        (() => {
    ...
    try {
        ...
        // ignore errors when forecast file not present or malformed
    }
    ...
    if (series.length === 0) {
        ...
        throw new Error(`Eurostat returned no observations for this topic (url: ${url}). ` +
